...
export const CreateSellerModal: React.FC<CreateSellerModalProps> = ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (isExistingEntity) {
        // Если организация уже существует, просто создаем связь
        createRelation(
          {
          ...
          },
          {
            ...
            onError: () => {
              //   message.error(`Ошибка при добавлении организации`);
            },
          }
        );
      } else {
        // Если организации нет, сначала создаем ее, затем связь
        createEntity(
          {
            ...values,
            inn,
          ...
          },
          {
            onSuccess: (data) => {
              ...
              //     {
              //       onSuccess: () => {
              //         // message.success("Организация успешно создана и добавлена");
              //         onSuccess();
              handleCancel();
              //       },
              //       onError: () => {
            ...
            onError: (error) => {
              //   message.error(
              // `Ошибка при создании организации: ${error.message}`
              //   );
            },
          }
        );
            ...
            {!isExistingEntity && (
              <>
                ...
                <Form.Item
                  ...
                  label="ОПФ"
                  rules={[
                    { required: true, message: "Пожалуйста, введите ОПФ" },
                  ]}
                >

...
export const CreateBuyerModal: React.FC<CreateBuyerModalProps> = ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (isExistingEntity) {
        // Если контрагент уже существует, просто создаем связь
        createRelation(
          {
        ...
        // Если контрагента нет, сначала создаем его, затем связь
        createEntity(
          {
            ...values,
            inn,
          ...
          },
          {
            onSuccess: (data) => {
              createRelation(
                ...
                {
                  onSuccess: () => {
                    // message.success("Контрагент успешно создан и добавлен");
                    onSuccess();
                    handleCancel();
                  },
                  onError: () => {
            ...
            onError: (error) => {
              // message.error(
              // `Ошибка при создании контрагента: ${error.message}`
              // );
            },
          }
        );
